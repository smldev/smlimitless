...
{
	/// <summary>
	///   Searches for files in a content package given the file name of the
	///   resource. Also caches search results in order to reduce processing time.
	/// </summary>
	internal sealed class ContentPackageResourceSearcher
	{
		/// <summary>
		///   A dictionary that maps resource names (keys) to file paths (values).
		/// </summary>
		...
		///   cref="ContentPackageResourceSearcher" /> class.
		/// </summary>
		...
		internal string GetResourcePath(string resourceName)
		{
			if (!filePaths.ContainsKey(resourceName))
			{
				...
				string[] matchingFilePaths = Directory.GetFiles(packagePath, string.Concat(resourceName, ".*"), SearchOption.AllDirectories).Where(s => !s.ToLowerInvariant().EndsWith(".txt")).ToArray(); // we need to exclude TXT configuration files
