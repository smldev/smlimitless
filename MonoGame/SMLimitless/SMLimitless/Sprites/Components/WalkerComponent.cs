...
namespace SMLimitless.Sprites.Components
{
	...
	public sealed class WalkerComponent : SpriteComponent
	{
		...
		/// <summary>
		///   Gets a flag indicating whether this sprite turns when it collides
		///   with another sprite.
		/// </summary>
		...
		///   The distance the owner sprite would need to be moved to resolve the collision.
		/// </param>
		...
		public override void Update()
		{
			if (TurnOnCliffs)
			{
				...
				{
					// There's a tile beneath this sprite. Check if we're near
					// the edge.
					int tileLeft = (int)tileBeneathOwner.Hitbox.Bounds.Left;
					int tileRight = (int)tileBeneathOwner.Hitbox.Bounds.Right;

					// Check to see if there's any other top-solid tile next to
					// this tile in the direction we're travelling. If not, turn.
