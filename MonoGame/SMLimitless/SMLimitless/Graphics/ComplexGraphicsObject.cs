...
{
	/// <summary>
	///   A collection of named graphics objects that can be switched between at will.
	/// </summary>
	public class ComplexGraphicsObject : IGraphicsObject
	{
		...
		public string CurrentObjectName
		{
			...
			set
			{
				...
				{
					throw new ArgumentException(string.Format("ComplexGraphicsObject.CurrentObjectName.set: There is no object named {0} loaded. Please check the config at {1}.", value, configFilePath));
				}
			}
		}
		...
		/// <summary>
		///   If the current object is animated, this method calls through to the
		///   animated object's AdjustSpeed(float) method. Adjusts the animation
		///   speed of any animated object.
		/// </summary>
		/// <param name="percentage">
		///   The percentage by which to adjust the speed.
		/// </param>
		...
		/// <summary>
		///   Returns a deep copy of this ComplexGraphicsObject. With the
		///   texture, only its reference is copied.
		/// </summary>
		...
		/// <param name="color">
		///   The color to shade the object. Use Color.White for no shading.
		/// </param>
		...
		/// </param>
		/// <param name="debug">
		///   If true, and the current object is an AnimatedGraphicsObject, the
		///   animated object's current frame number is drawn at the top-left
		...
		/// <summary>
		///   Draws the currently active object, with cropping, in this ComplexGraphicsObject.
		/// </summary>
		/// <param name="position">The position at which to draw the object.</param>
		/// <param name="cropping">The portion of the texture to draw.</param>
		/// <param name="color">
		///   The color to shade the object. Use Color.White for no shading.
		/// </param>
		...
		/// <summary>
		///   Gets a graphic suitable for display in the level editor's object
		///   selection window.
		/// </summary>
		/// <returns>
		///   A <see cref="Texture2D" /> instance containing the graphic to display.
		/// </returns>
		...
		public Vector2 GetSize()
		{
			if (isContentLoaded)
			{
				IGraphicsObject graphicsObject = graphicsObjects.First().Value;
		...
		/// <summary>
		///   Loads a ComplexGraphicsObject. This overload is only present to
		///   fulfill the IGraphicsObject contract. Do not call it, call the
		///   Load(string, DataReader) overload instead.
		/// </summary>
		...
		public void LoadContent()
		{
			if (isLoaded && !isContentLoaded)
			{
				...
				foreach (IGraphicsObject graphicsObject in graphicsObjects.Values)
				{
					...
					{
						throw new InvalidCastException(string.Format("ComplexGraphicsObject.LoadContent(): Unrecognized graphics object type. Please check config at {0}.", configFilePath));
					}
				}
		...
		/// <summary>
		///   If the current object is animated, this method calls through to the
		///   animated object's Reset method. Resets the object, and optionally
		///   starts the animation running again.
		/// </summary>
		/// <param name="startRunning">
		///   If true, the object will begin running again.
		/// </param>
		...
		/// <summary>
		///   If the current object is animated, this method calls through to the
		///   animated object's SetSpeed(decimal) method. Adjusts the cycle
		///   length of any animated object.
		/// </summary>
		...
		/// <summary>
		///   If the current object is animated, this method calls through to the
		///   animated object's SetSpeed method. Adjusts the frame time of any
		///   animated object.
		/// </summary>
